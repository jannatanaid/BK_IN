...
class BookInventory:
    ...
    def remove_book(self, index):
        ...
        if current:
            ...
            print(f"The book {index} not found in the inventory.")
...
class AuthSystem:
    ...
    def input_with_validation(self, prompt, validation_error, validator=None):
        while True:
            user_input = input(prompt).strip()
            if not user_input:
                print(validation_error)
            elif validator and not validator(user_input):
                print("Error: Invalid input.")
            else:
                return user_input

    def validate_username(self, username):
        # Check if the username contains only symbols
        return any(char for char in username if char not in string.punctuation)

    def register(self):
        while True:
            username = self.input_with_validation("Set up a username: ", "Error: Username cannot be empty or just spaces.", self.validate_username)
